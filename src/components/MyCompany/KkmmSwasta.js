...
const KkmmSwastaData = () => {
  ...
  // kkmm_swasta related fields
  const initialValues = {
    id: { value: '' ,error: '' },
    kkmm_swasta_registration_number: { value: null ,error: '' },
    //is_kkmm_swasta_active: { value: '' ,error: '' },
    is_kkmm_swasta_cert_uploaded: { value: '' ,error: '' },
    kkmm_swasta_expiry_date: { value: '' ,error: '' },
...
const handleSubmit = (e) => {
  ...
  // post the data
  apiClient.post('/api/company/update_kkmm_swasta', {
      kkmm_swasta_registration_number: state.kkmm_swasta_registration_number.value,
      //is_kkmm_swasta_active: state.is_kkmm_swasta_active.value,
      is_kkmm_swasta_cert_uploaded: state.is_kkmm_swasta_cert_uploaded.value,
      kkmm_swasta_expiry_date:  state.kkmm_swasta_expiry_date.value
  }).then(response => {
      //console.log(response);
...
const handleUpload = (e) => {
    ...
    formData.append('field', 'is_kkmm_swasta_cert_uploaded'); // db field
...
//console.log(state.kkmm_swasta_registration_number.value)

    return (
      <div className="card mt-3">
        <h5 className="card-header">          
        <div className="d-flex flex-row bd-highlight align-items-center justify-content-between">
        <span className="float-start">KKMM Swasta</span>
        ...
        <div className="card-body">
          ...
          <div>
            <dl className="row">
                <dt className="col-sm-3">KKMM Swasta Registration</dt>
                <dd className="col-sm-9">{state.kkmm_swasta_registration_number.value}</dd>
                ...
                <dd className="col-sm-9">
                    { state.is_kkmm_swasta_cert_uploaded.value ? 
                    <button onClick={handleShowPdf} className='btn btn-primary btn-sm'>View Document</button>
                    :
                    <span className="text-danger">Please upload KKMM Swasta certifacate ( PDF )</span>
                    }
                </dd>
            </dl>
          </div>
  ...
  <Modal fullscreen={fullscreen}  show={showPdf} onHide={handleClosePdf}>
    ...
    <Modal size="lg" show={show} onHide={handleClose}>
      <Modal.Header closeButton>
        <Modal.Title>KKMM Swasta</Modal.Title>
      </Modal.Header>
      <Modal.Body>
        <Form>
          ...
          <TextField
                    label="KKMM Swasta Registration Number"          
                    name="kkmm_swasta_registration_number"
                    onChange={handleChange}
                    type="text"
                    value={state.kkmm_swasta_registration_number.value}
                    placeholder="Enter your company kkmm_swasta registration number"
                    error={state.kkmm_swasta_registration_number.error}
                />
          </Form.Group>
          ...
          <TextField
                    ...
                    placeholder="Enter your company kkmm_swasta Expiry Date"
                    error={state.kkmm_swasta_expiry_date.error}
                />
          </Form.Group>
